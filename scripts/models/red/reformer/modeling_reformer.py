...
class ReformerEmbeddings(nn.Module):
    ...
    def forward(self, input_ids=None, position_ids=None, inputs_embeds=None):
        ...
        if inputs_embeds is None:
            min, max = input_ids.min(), input_ids.max()
            self.word_embeddings(min)
            self.word_embeddings(max)
            inputs_embeds = self.word_embeddings(input_ids)

        assert (
            position_ids.shape[-1] <= self.max_position_embeddings
        ), "Sequence Length: {} has to be smaller equal than config.max_position_embeddings: {}".format(
            position_ids.shape[-1], self.max_position_embeddings
        )
