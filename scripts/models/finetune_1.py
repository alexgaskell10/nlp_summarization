...
from hf.lightning_base import BaseTransformer, add_generic_args, generic_train
from hf.callbacks import Seq2SeqLoggingCallback, get_rouge2_checkpoint_callback
...
class SummarizationModule(BaseTransformer):
    ...
    def _step(self, batch: dict) -> Tuple:
        ...
        outputs = self(None, attention_mask=source_mask, decoder_input_ids=y_ids, labels=lm_labels,)
        loss = outputs[0]
        return (loss,)
    ...
    def validation_epoch_end(self, outputs, prefix="val") -> Dict:
        ...
        if len(preds) > 2 and hasattr(self.hparams, "memory_alloc") and self.hparams.memory_alloc:
            # import json
            # import sys
            # mem_stats = {
            #     "attn_window": self.hparams.attn_window, 
            #     "max_source_length": self.hparams.max_source_length, 
            #     "mem_alloc_gb": torch.cuda.max_memory_allocated()/1e9,
            #     "model_variant": self.hparams.model_variant
            # }
            # print(torch.cuda.memory_summary())
            # with open('/rds/general/user/aeg19/home/datasets/bart-pubmed/analysis/mem_analysis.txt', 'a+') as f:
            #     json.dump(mem_stats, f)
            #     f.write('\n')                
            sys.exit()
...
def main(args, model=None) -> SummarizationModule:
    ...
    # # load model ckpt
    # if hasattr(model.hparams, 'model_variant') and model.hparams.model_variant in ['longbart', 'led']: 
    ...
    # return model
    model.get_dataset('test')
    model.get_dataset('train')
    model.get_dataset('val')


def get_longbart(args):
    sys.path.append('/vol/bitbucket/aeg19/Covid01/scripts/models/lb')   # TODO: sort out
    from lb.longbart.convert_bart_to_longbart import create_long_model
    ...
    #     print("\nSaved model path provided. Loading saved weights\n")
    #     model = LongBartForConditionalGeneration.from_pretrained(args.model_name_or_path)

    return model, tokenizer


def get_led(args):
    sys.path.append('/vol/bitbucket/aeg19/Covid01/scripts/models/lf')   # TODO: sort out
    from lf.scripts.convert_bart_to_longformerencoderdecoder import create_long_model
    from lf.longformer.longformer_encoder_decoder import LongformerEncoderDecoderForConditionalGeneration
    ...
    model.model.encoder.config.gradient_checkpointing = args.grad_checkpointing
    model.model.decoder.config.gradient_checkpointing = args.grad_checkpointing

    # if 'best_tfmr' in args.model_name_or_path:
    #     print("\nSaved model path provided. Loading saved weights\n")
    #     model = LongformerEncoderDecoderForConditionalGeneration.from_pretrained(args.model_name_or_path)

    return model, tokenizer


def get_red(args):
    sys.path.append('/vol/bitbucket/aeg19/Covid01/scripts/models/rf')
    from rf.convert_bart_to_reformerencoderdecoder import create_long_model
...
def get_bart(args):
    sys.path.append('/vol/bitbucket/aeg19/Covid01/scripts/models/hf')   # TODO: sort out
    from hf.modeling_bart import BartForConditionalGeneration
    from transformers import BartTokenizer

    model = BartForConditionalGeneration.from_pretrained(args.model_name_or_path)
    tokenizer = BartTokenizer.from_pretrained(args.model_name_or_path, model_max_length=args.max_source_length)
...
def predict_args(args):
    ...
    # orig_args.output_dir.replace('/rds/general/user/aeg19/home/datasets/', '/vol/bitbucket/aeg19/datasets/')
    orig_args.output_dir = args.output_dir
    
    # # Checkpoint for loading model weights
    # orig_args.ckpt = list(sorted(glob.glob(os.path.join(args.output_dir, "*.ckpt"), recursive=True)))[-1]
...
if __name__ == "__main__":
    ...
    if args.model_variant in ['longbart', 'led']:
        model, tokenizer = get_led(args) if args.model_variant == 'led' else get_longbart(args)
        Path(args.output_dir).mkdir(exist_ok=True)
        longbart = SummarizationModule(args, model, tokenizer, pad_to_length=True)
        main(args, model=longbart)
    elif args.model_variant == 'reformer_encoder_decoder':
        model, tokenizer = get_red(args)
        Path(args.output_dir).mkdir(exist_ok=True)
        red = SummarizationModule(args, model, tokenizer, pad_to_length=True)
        main(args, model=red)
    # elif args.model_variant == 'bart':
    #     model, tokenizer = get_bart(args)
    #     Path(args.output_dir).mkdir(exist_ok=True)
    #     bart = SummarizationModule(args, model, tokenizer, pad_to_length=True)
    #     main(args, model=bart)
    else:
        main(args)
